...
- `RouteSegment.geometry` — les `LineString` par segment, déjà au contrat depuis
  UF-403. **Aucun changement d'API n'a été nécessaire pour ce ticket.**
...
| **C5**     | L'abonnement haute précision ne vit que dans `guiding` et `signal-lost` (`needsPositionWatch`) ; **zéro requête réseau** pendant tout le guidage                                                                              |
...
**Il n'écrit rien côté serveur.** Le trajet effectivement parcouru n'est pas
remonté : c'est l'objet d'UF-807 (« trajet réalisé ≠ intention »). Aujourd'hui,
le suivi carbone compte toujours l'option _retenue_ au moment du clic
(`PATCH` sélection, UF-505), que le guidage soit allé au bout ou non.
